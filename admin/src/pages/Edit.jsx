  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div>
          ...
          <div className='flex gap-3'>
            {["Yellow", "Light Blue", "Green"].map((color) => (
              <div key={color} onClick={() => setSizes(prev => prev.includes(color) ? prev.filter(item => item !== color) : [...prev, color])}>
                <p className={`${sizes.includes(color) ? "bg-pink-100" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>{color}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className='flex gap-3'>
            {["Lavender", "Vanilla", "Rose"].map((fragrance) => (
              <div key={fragrance} onClick={() => setSizes(prev => prev.includes(fragrance) ? prev.filter(item => item !== fragrance) : [...prev, fragrance])}>
                <p className={`${sizes.includes(fragrance) ? "bg-pink-100" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>{fragrance}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className='flex gap-4 items-center'>
        <input onChange={() => setBestseller(!bestseller)} type="checkbox" id='bestseller' checked={bestseller} />
